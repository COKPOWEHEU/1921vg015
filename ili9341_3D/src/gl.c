...
#define BSYLED A,12,1,GPIO_PP //GLED1
...
#define GL_MAXX	(LCD_MAXX + 1)
#define GL_MAXY	(LCD_MAXY - 20)

uint16_t gl_buffer[2*GL_MAXY];
uint16_t *bgbuffer = gl_buffer;
int16_t zbuf[GL_MAXY];
uint16_t gl_x=1;
...
#if GL_MAXX > GL_MAXY
  #define LCD_MIN2 (GL_MAXY/2)
#else
  #define LCD_MIN2 (GL_MAXX/2)
#endif
/*
...
void glIntTransform(const struct glVector3 *src, struct glVector3int *res){
  ...
  temp.x = temp.x*LCD_MIN2 + (GL_MAXX/2);
  temp.y = temp.y*LCD_MIN2 + (GL_MAXY/2);
...
void glDrawTriangleV(const struct glVector3 *t1, const struct glVector3 *t2, const struct glVector3 *t3, const struct glVector3 *n){
  ...
  uint16_t r = (t1->col >> 8)&0b11111000;
  uint16_t g = (t1->col >> 3)&0b11111100;
  uint16_t b = (t1->col << 3)&0b11111000;
  r = (r*cosa); r &= 0b11111000;
  g = (g*cosa); g &= 0b11111100;
  b = (b*cosa); b &= 0b11111000;
  
  t->col = rgb2col(r, g, b);
  t->col = (t->col >> 8) | (t->col << 8);
...
void glIntDrawTriangle(struct glTriangle *t){
  ...
  if(y1<0)y1=0;
  if(y1>=GL_MAXY)y1=GL_MAXY-1;
  if(y2<0)y2=0;
  if(y2>=GL_MAXY)y2=GL_MAXY-1;
...
void glSwapBuffers(){
  ...
  bgbuffer=gl_buffer;
  glIntDraw();
  bgbuffer=gl_buffer+GL_MAXY;

  lcd_wait();
  lcd_begin_area(0,0, GL_MAXX-1, GL_MAXY-1);
  lcd_dma_init((void*)gl_buffer,GL_MAXY);
  
  for(gl_x=1;gl_x<GL_MAXX;gl_x++){
    glIntDraw();
    GPO_ON(BSYLED);
    lcd_wait();
    GPO_OFF(BSYLED);
    lcd_dma_init((void*)bgbuffer,GL_MAXY);
    
    if(bgbuffer == gl_buffer)bgbuffer=gl_buffer+GL_MAXY;
